...
**Status:** HISTORICAL; ROUTE-LEVEL CASES SUPERSEDED BY GENERIC QUEST/ENCOUNTER FLOW
...
|---|---|---|---|---|---|
| ACF-01 | Open an unenrolled course | Historical Course Flow case; accepted opportunities and enrollment now enter through professor quests and Journal | integration | generic quest coverage | Superseded |
| ACF-02 | Open an active or completed course | Historical Course Flow case; active/completed state now belongs to Journal history | integration | generic quest coverage | Superseded |
...
| ACF-10 | Win an activity with a selectable reward | Combined Results presents inspectable options and progress pauses until a valid selection is authoritatively confirmed | integration | `tests/unit/meta/test_post_battle_results.gd` | Implemented |
...
| ACF-14 | Navigate back from encounter Preparation | Shared back icon returns to the correct parent context and exposes an accessible label | integration | `tests/unit/meta/test_encounter_flow.gd` | Implemented |
...
| ACF-16 | Search the finished runtime for legacy paths | No old course modal, Collection launch detour, inferred loadout mode, or legacy loadout fields remain | structural | `tests/unit/meta/test_encounter_flow.gd` | Implemented |
| ACF-17 | Inspect a locked future activity | Historical course-screen case; future opportunities are now represented through generic quest/Journal discovery | integration | generic quest coverage | Superseded |
...
| ACF-24 | Render activity type information | UI consumes authoritative typed/view-model fields without comparing legacy activity-type strings | structural + integration | `tests/unit/meta/test_encounter_flow.gd` | Implemented |

...
**Description:**
Add GUT (Godot Unit Test) framework for automated testing of game services and logic. This enables regression testing and CI/CD integration.

**Requirements:**
- Install GUT addon from AssetLib
- Set up test directory structure (`tests/unit/`, `tests/integration/`)
- Create base test fixtures for common scenarios
- Add CI script to run tests on PR

**Priority Test Candidates (based on current architecture):**

1. **BattleContext Tests** (High Value - state machine logic)
   - State transitions: NONE → CONFIGURED → IN_PROGRESS → VICTORY/DEFEAT/ABANDONED
   - `abandon_battle()` clears profile state correctly
   - `clear()` resets all fields
   - Configuration methods set correct state

2. **CampaignService Tests** (High Value - progression logic)
   - `complete_battle()` adds to completed list and saves
   - `is_battle_completed()` returns correct values
   - `is_battle_unlocked()` respects unlock requirements
   - `claim_pending_reward()` grants cards and marks complete
   - Pending reward persistence across sessions

3. **ProfileRepository Tests** (High Value - data integrity)
   - Profile creation with correct schema
   - Save/load round-trip integrity
   - WAL operations apply correctly
   - Backup rotation works

4. **Card System Tests**
   - Card creation from catalog
   - Mana cost validation
   - Card effect application

5. **Economy Tests**
   - Gold add/subtract/check
   - Resource validation

**Notes:**
- Focus on service layer first (pure logic, few dependencies)
- UI testing is harder and lower priority
- Consider mocking ProfileRepo for tests that don't need persistence

---

*Last Updated: 2025-11-28 - Fixed battle exit bug, added testing framework todo*

...
**Description:**
28 tests in `test_campaign_service.gd` fail because the CampaignService was migrated to a hybrid GDScript/C# architecture. Tests create standalone instances with `CampaignServiceScript.new()`, but `_cs_service` (CampaignServiceCS autoload) is null.

**Root Cause:**
1. Test creates instance not added to scene tree
2. `_ready()` calls `get_node_or_null("/root/CampaignServiceCS")` which returns null
3. `init_for_testing()` doesn't inject a mock C# service
4. All methods check `if _cs_service == null: return []`

**Impact:**
Tests fail but production code works correctly. Tests were written before C# migration.

**Proposed Fix Options:**
1. Refactor tests to use the autoloaded `Campaign` service instead of creating new instances
2. Add `_cs_service` injection to `init_for_testing()` with a mock C# service
3. Add test instance to scene tree so it can find the autoload
...
**Description:**
3 tests fail and 7 are skipped in `test_shop_ownership.gd` because `Shop.get_shop_offerings("premium_store")` returns an empty array, even though logs show "ShopService: Initialized 3 shops".

**Failing Tests:**
- `test_premium_store_has_summoner_offerings` - expects > 0, gets 0
- `test_premium_store_has_cosmetic_offerings` - expects > 0, gets 0
- `test_premium_store_has_emote_offerings` - expects > 0, gets 0

**Potential Causes:**
1. Timing issue: `before_each()` calls `ProfileRepo.load_profile()` and `ProfileRepo.reset_profile()` which may affect shop state
2. Shop dictionary `_shops["premium_store"]` not populated at query time
3. `_build_offering_from_dict()` silently failing

**Impact:**
Tests fail but production shop functionality works correctly.

**Investigation Needed:**
- Add debug logging to trace why offerings aren't returned
- Check if profile operations affect shop initialization
- Verify `_shops` dictionary state when tests query it

**Related Files:**
- tests/unit/test_shop_ownership.gd
- scripts/services/shop_service.gd

---

*Last Updated: 2026-01-23 - Added test infrastructure bugs for CampaignService and ShopService*

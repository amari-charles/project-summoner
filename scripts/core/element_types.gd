...
func is_core(element: Variant) -> bool:
	if element is Element:
		...
		var elem: Element = from_string(element)
...
func get_display_name(element: Variant) -> String:
	if element is Element:
		...
		var elem: Element = from_string(element)
		return elem.display_name if elem != null else element.capitalize()
	return str(element)
...
func can_elevate(element: Variant) -> bool:
	var elem: Element = element if element is Element else from_string(element)
	if elem == null:
		return false

...
func _render_progression_tree() -> void:
	...
	var root_traits: Array = _sorted_traits_by_name(grouped_by_depth.get(0, []))
	var root_entries: Array = []
	for index: int in range(root_traits.size()):
		var root_node_data: Dictionary = root_traits[index]
	...
	for node_var: Variant in _progression_nodes:
		...
		for prereq_var: Variant in prerequisites:
			...
			edges.append(_build_edge(from_node, to_node, _edge_color_for(prereq_id, trait_id), 2.8))
...
func _on_trait_node_pressed(trait_id: String) -> void:
	...
	var detail: Dictionary = TraitTreeApi.get_trait_node_detail("summoner", _summoner_id, trait_id)
	if detail.is_empty():
		return
...
func _on_unlock_confirmed() -> void:
	if _summoner_id.is_empty() or _pending_unlock_trait_id.is_empty():
		return

	var trait_id: String = _pending_unlock_trait_id
	_pending_unlock_trait_id = ""

	var result: Dictionary = TraitTreeApi.try_unlock_trait("summoner", _summoner_id, trait_id)

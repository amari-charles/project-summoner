...
func _on_card_selected(catalog_id: StringName) -> void:
	...
	var card_instance_id: String = CardServiceApi.grant_card(catalog_id, String(RarityIDs.COMMON))
	...
	if card_instance_id != "":
		...
		if Decks.has_method("ListDecksDict"):
			var all_decks: Array[Dictionary] = DecksApi.list_decks_dict()
		...
		if deck_id != "":
			if Decks.has_method("AddCardToDeck"):
				DecksApi.add_card_to_deck(deck_id, card_instance_id)
		else:
...
func _get_first_unlocked_summoner() -> String:
	var unlocked: Variant = SummonerSelectionApi.get_unlocked_summoner_ids_array()
	if unlocked.size() > 0:
		return unlocked[0]
	return ""

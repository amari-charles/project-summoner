...
func _ready() -> void:
	...
	loadout_label.text = Loc.t("academy.flow.battle_loadout")
...
func _refresh() -> void:
	...
	var reward_count: int = SafeTypeUtils.array(_state.get("reward_previews")).size()
	details_label.text = "%s • %s\n%s\n%s\n%s" % [
		...
		Loc.t("academy.flow.rewards", {"rewards": Loc.t("academy.flow.none") if reward_count == 0 else Loc.t("academy.flow.rewards_visible", {"count": reward_count})}),
		Loc.t("academy.flow.loadout_valid") if SafeTypeUtils.bool_val(validation.get("is_valid")) else _validation_issue_summary(issues),
	]
...
func _render_loadout() -> void:
	...
	for value: Variant in SafeTypeUtils.array(loadout.get("supplied_cards")):
		...
		_add_card_button(loadout_grid, card, true, Callable())
	for value: Variant in SafeTypeUtils.array(loadout.get("selected_cards")):
		var card: Dictionary = SafeTypeUtils.dict(value)
		_add_card_button(loadout_grid, card, false, _toggle_class_card.bind(SafeTypeUtils.string(card.get("card_instance_id"))))
	if mode == "ClassLoadout":
		for value: Variant in SafeTypeUtils.array(loadout.get("available_cards")):
			var card: Dictionary = SafeTypeUtils.dict(value)
			_add_card_button(available_grid, card, SafeTypeUtils.bool_val(card.get("selected")), _toggle_class_card.bind(SafeTypeUtils.string(card.get("card_instance_id"))))
	elif mode == "Owned":
...
func _add_card_button(parent: Control, card: Dictionary, locked: bool, action: Callable) -> void:
	...
	button.text = "%s%s x%d" % ["🔒 " if locked else "", SafeTypeUtils.string(card_data.get("card_name"), card_id), count]

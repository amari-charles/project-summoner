...
public partial class EconomyService : Node
{
    ...
    {
        if (_profileRepo == null || !summonerId.HasValue)
            return 0;
    ...
    public void AddCampaignGold(int amount, SummonerId summonerId)
    {
        ...
        {
            GD.PushWarning(
                $"EconomyService: AddCampaignGold called with non-positive amount: {amount}"
            );
            return;
        }
        ...
        {
            GD.PushWarning(
                "EconomyService.AddCampaignGold: No summoner specified and no active summoner"
            );
            return;
        }
        ...
        {
            GD.PushWarning(
                $"EconomyService.AddCampaignGold: No campaign progress for summoner '{summonerId}'"
            );
            return;
        }
        ...
        GD.Print(
            $"EconomyService: Added {amount} campaign gold to '{summonerId}' (now: {progress.Gold})"
        );
    ...
    public bool SpendCampaignGold(int amount, SummonerId summonerId)
    {
        ...
        GD.Print(
            $"EconomyService: Spent {amount} campaign gold from '{summonerId}' (now: {progress.Gold})"
        );
        EmitSignal(SignalName.CampaignGoldChanged, summonerId.Value, progress.Gold);
        return true;
    }

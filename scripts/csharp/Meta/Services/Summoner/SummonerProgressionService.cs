...
public partial class SummonerProgressionService : Node
{
    ...
    public int GrantSummonerXp(SummonerId summonerId, int amount)
    {
        ...
        {
            GD.PushWarning(
                $"SummonerProgressionService: Summoner instance not found: {summonerId}"
            );
            return 0;
        }
    ...
    public Godot.Collections.Dictionary GetSummonerProgressionInfo(SummonerId summonerId)
    {
        ...
        return new Godot.Collections.Dictionary
        {
            ...
            ["xp_for_next_level"] = ProgressionEngine.GetXpCostForNextLevel(
                BuildProgressionState(summoner),
                SharedProgressionCurve
            ),
    ...
    public int GrantTraitPoints(SummonerId summonerId, int amount, string source = "")
    {
        ...
        if (!string.IsNullOrEmpty(source))
            GD.Print(
                $"SummonerProgressionService: Granted {amount} trait points to '{summonerId}' from source='{source}'"
            );

        return summoner.UnspentTraitPoints;
    }

    public Godot.Collections.Array<Godot.Collections.Dictionary> RollTraitOffers(
        string summonerId,
        int count = 3
    )
    {
        ...
        var ordered = eligible
            .OrderBy(trait =>
                ComputeStableOfferOrder(
                    $"{typedSummonerId.Value}|{evaluationLevel}",
                    trait.Id.Value
                )
            )
    ...
    public bool SpendTraitPoint(SummonerId summonerId, string traitId)
    {
        ...
        if (summoner == null)
            return false;
        if (summoner.UnspentTraitPoints <= 0)
            return false;
    ...
    private static TraitTreeOwnerContext BuildTraitOwnerContext(
        ...
        SummonerDefinition summonerDef
    )
    {

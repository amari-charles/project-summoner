...
/// Loaded from JSON via ContentCatalog.
///
/// Tuning Guide:
...
public class ProjectileData
{
    ...
    /// <summary>
    /// Create ProjectileData from a GDScript ProjectileData resource.
    /// </summary>
    public static ProjectileData FromGodotResource(GodotObject resource)
    {
        var data = new ProjectileData
        {
            ProjectileId = resource.Get("projectile_id").AsString(),
            ProjectileName = resource.Get("projectile_name").AsString(),
            ModelScenePath = resource.Get("model_scene_path").AsString(),
            TrailVfx = GetResourceString(resource, "trail_vfx", GetResourceString(resource, "trail_effect_id")),
            HitVfx = GetResourceString(resource, "hit_vfx", GetResourceString(resource, "impact_effect_id")),
            Speed = resource.Get("speed").AsSingle(),
            Acceleration = resource.Get("acceleration").AsSingle(),
            MinSpeed = resource.Get("min_speed").AsSingle(),
            ...
            AoeRadius = resource.Get("aoe_radius").AsSingle(),
            LaunchSound = resource.Get("launch_sound").AsString(),
            ImpactSound = resource.Get("impact_sound").AsString()
        };

        // Parse movement type
        var movementStr = resource.Get("movement_type").AsString().ToLower();
        data.MovementType = movementStr switch
        {
            "homing" => ProjectileMovementType.Homing,
            "arc" => ProjectileMovementType.Arc,
        ...
        // Get visual scene
        var visualSceneVar = resource.Get("visual_scene");
        if (visualSceneVar.VariantType != Variant.Type.Nil)
        {
            data.VisualScene = visualSceneVar.As<PackedScene>();
        }
    ...
    private static string GetResourceString(GodotObject resource, string property, string defaultValue = "")
    {

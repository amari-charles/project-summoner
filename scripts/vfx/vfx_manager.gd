...
var effects_container: Node3D  ## Parent for all effects
		...
		if vfx_def.pooled and vfx_def.effect_scene:
			...
			for i in range(vfx_def.pool_size):
				...
				if instance:
					...
					instance.reset()
					effect_pools[effect_id].append(instance)
...
func play_effect_2d(effect_id: String, position: Vector2, data: Dictionary = {}) -> VFXInstance:
	...
	var pos_3d = Vector3(position.x, position.y, 0)
...
func _get_from_pool(effect_id: String) -> VFXInstance:
	...
	var pool = effect_pools[effect_id]
	if pool.size() > 0:
...
func _on_effect_finished(effect_id: String, instance: VFXInstance) -> void:
	...
	if active_effects.has(effect_id):
		active_effects[effect_id].erase(instance)
...
func print_pool_stats() -> void:
	...
	for effect_id in effect_pools.keys():
		var pool_size = effect_pools[effect_id].size()
		var active_size = active_effects[effect_id].size() if active_effects.has(effect_id) else 0

...
func test_results_presents_authoritative_grants_without_mutating_xp() -> void:
	...
	assert_true(script_text.contains("button.text = _option_label(option)"))
	assert_true(script_text.contains("displayed_grants.append_array(selected_grants)"))
	assert_true(script_text.contains("show_description = false"))
	assert_false(script_text.contains("show_description = true"))

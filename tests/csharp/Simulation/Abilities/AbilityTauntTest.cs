...
public class AbilityTauntTest
{
    ...
    public void TauntPulse_UsesSoftOverride_WhenExistingForcedTargetIsActive()
    {
        ...
        enemyB.ForcedTargetUnitId = 9999;
        enemyB.ForcedTargetTimer = 1.5f;

        SimAbilityOrchestrator.Tick(state, Simulation.FixedDeltaSeconds, events);

        AssertThat(enemyA.ForcedTargetUnitId.HasValue).IsTrue();
        AssertThat(enemyA.ForcedTargetUnitId ?? -1).IsEqual(taunter.UnitId);
        AssertThat(enemyA.ForcedTargetTimer).IsGreater(0f);
        AssertThat(enemyB.ForcedTargetUnitId.HasValue).IsTrue();
        AssertThat(enemyB.ForcedTargetUnitId ?? -1).IsEqual(9999);
